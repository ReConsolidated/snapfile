...
public class DownloadRequestService {
    ...
    public List<DownloadRequest> getRequests(String code) {
        return downloadRequestRepository.findByCodeEqualsAndAcceptedIsFalse(code);
    }

    public DownloadRequest requestDownload(String id, String name, String code) {
        Optional<CodeInstance> codeInstance = codeManagementService.getCodeInstance(code);
        if (codeInstance.isPresent()) {
            return downloadRequestRepository.save(new DownloadRequest(id, name, code));
        }
